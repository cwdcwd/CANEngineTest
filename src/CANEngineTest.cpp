...
byte data[8] = {0x02, SERVICE_CURRENT_DATA, PID_FUEL_RATE, 0xff, 0xff, 0xff, 0xff, 0xff};
...
void loop()
{
    if (!digitalRead(CAN0_INT)) // If CAN0_INT pin is low, read receive buffer
    {
      ...
      if(len>0) {
        ...
        { // Determine if message is a remote request frame.
          ...
          Log.trace(msgString);
          // Particle.publish("can-data-event", msgString);
        }
      } else {
        Log.trace(".");
      }
    ...
    if (millis() - lastRequest >= 5000)
    {
      ...
      if (sndStat == CAN_OK)
      {
        ...
        // Particle.publish("can-data-req", "sent to: " + String(OBD_CAN_REQUEST_ID));
      }
      else
      {
        Log.error("Error Sending Message %d", sndStat);
        // Particle.publish("can-data-err", "could not send data to: " + String(OBD_CAN_REQUEST_ID));
      }
      lastRequest = millis();
    }
}
